...
class MdSeiRestAiIntegracao extends SeiIntegracao {
	
	public function getNome()
	{
		return 'SEI REST AI - Integração REST + Inteligência Artificial';
	}
	...
	{
		return 'Módulo REST AI - Integração com APIs REST e Inteligência Artificial para SEI';
	}
	...
	public function montarMenuUsuarioExterno()
	{
		...
		if($bolMenuUsuarioExterno){
			...
			if(is_array($arrModulos) && array_key_exists('PesquisaIntegracao', $arrModulos)){
				...
				$arrMenu = array();
				$arrMenu[] = '-^'.ConfiguracaoSEI::getInstance()->getValor('SEI','URL').'/modulos/'.$caminho.'/md_pesq_processo_pesquisar.php?acao_externa=protocolo_pesquisar&acao_origem_externa=protocolo_pesquisar^^Pesquisa P�blica^_blank^';
				return $arrMenu;
			}
		}
		return null;
	...
	/**
	 * FUNCTION ADICIONADAS PARA ATENDER A MIGRA��O DOS PARAMETROS DO M�DULO UTILIDADES PARA O PESQUISA P�BLICA
	 */
	
	/**
	 *  Verifica se est� na �ltima conclus�o
	 * @param $idProtocolo
	 * @return bool
	 */
	...
	/**
	 * Verifica se Existe documentos restritos e se o valor da hipotese legal est� setada no par�mentro
	 * @param $documentos
	 * @param $arrValor
	 * @return string
	 */
	...
	public function bloquearProcesso($objProcedimentoAPI)
	{
		...
		if (!empty($strValor) && $countOjAtividade == 1) {
			...
			if (!empty($listaDocumentos.$listaDocProcessoAnexo.$listaDocProcessoAnexo2.$listaMsgProcesso)) {
				$msg = "N�o � poss�vel bloquear o processo n ".$objPrcPrincipalDTO->getStrProtocoloProcedimentoFormatado().", pois nele ou em processo anexado ainda constam documentos com N�vel de Acesso Restrito usando as Hip�teses Legais abaixo: \n\n" . $listaDocumentos.$listaDocProcessoAnexo.$listaDocProcessoAnexo2.$listaMsgProcesso;
	...
	public function concluirProcesso($arrObjProcedimentoAPI)
	{
		...
		if (!empty($strValor) && $ultimaConclusao) {
			...
			// Caso o Processo esteja com restri��o por Hip�tese Legal
			if ($objPrcPrincipalDTO->getStrStaNivelAcessoLocalProtocolo() == ProtocoloRN::$NA_RESTRITO) {
				if (in_array($objPrcPrincipalDTO->getNumIdHipoteseLegalProtocolo(), $arrValor)) {
					$objHipotesePrincipalDTO = new HipoteseLegalDTO();
					$objHipotesePrincipalDTO->setNumIdHipoteseLegal($objPrcPrincipalDTO->getNumIdHipoteseLegalProtocolo());
			...
			if (!empty($listaDocumentos.$listaDocProcessoAnexo.$listaDocProcessoAnexo2.$listaMsgProcesso)) {
				$objInfraException = new InfraException();
				$msg = "N�o � poss�vel concluir o processo n� ".$objPrcPrincipalDTO->getStrProtocoloProcedimentoFormatado().", pois nele ou em processo anexado ainda constam documentos com N�vel de Acesso Restrito usando as Hip�teses Legais abaixo: \n\n" . $listaDocumentos.$listaDocProcessoAnexo.$listaDocProcessoAnexo2.$listaMsgProcesso;
